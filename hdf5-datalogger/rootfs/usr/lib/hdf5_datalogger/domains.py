...
def build_included_domains(include_domains_raw, available_domains, default_domains):
    """
    Ritorna (selected_domains, warnings):

    - selected_domains: set di domini da includere.
        * set vuoto => includi tutti i domini disponibili.
    - warnings: lista di stringhe da mostrare nel report/log.
    """
    warnings = []

    # Nessun include_domains configurato: usa i domini di default
    if not include_domains_raw:
        if not default_domains:
            return set(), warnings
        effective = set(default_domains) & set(available_domains)
        if not effective:
            warnings.append("Nessun dominio di default presente nel sistema; uso tutti i domini disponibili.")
            return set(), warnings
        return effective, warnings

    # Caso: include_domains configurato
    ...
    if not effective:
        ...
        return set(), warnings  # set vuoto => tutti i domini disponibili
